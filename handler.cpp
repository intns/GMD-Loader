...
std::vector<unsigned char> Handler::Read(uint32_t vaddress, size_t len) const
{
	std::vector<unsigned char> out(len);
	ReadProcessMemory(this->hProcess, reinterpret_cast<void*>(vaddress), out.data(), len, NULL);
	return out;
}
